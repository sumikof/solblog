...
declare_id!("5zMhWKsdPYNhocSf1Jhvr5AgtLw7seZwk6Q7ycQRVfsP");
...
pub mod my_app {
    ...
    pub fn signup_user(ctx: Context<SignupUser>,name: String, avator: String) -> Result<()> {
        let user_account = &mut ctx.accounts.user_account;
        let authority = &mut ctx.accounts.authority;

        user_account.name = name;
        user_account.avator = avator;
        user_account.authority = authority.key();

        Ok(())
    }

    pub fn update_user(ctx: Context<UpdateUser>,name: String, avator: String) -> Result<()> {
        let user_account = &mut ctx.accounts.user_account;
